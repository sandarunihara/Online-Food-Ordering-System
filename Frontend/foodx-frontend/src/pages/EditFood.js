...
const EditFood = () => {
  ...
  useEffect(() => {
    if (user && user.role === 'ROLE_RESTAURANT_OWNER' && id) {
      fetchData();
    }
  }, [user, id]);
  ...
  const fetchData = async () => {
    try {
      ...
      // Get food items and find the one to edit
      const foodResponse = await foodAPI.getByRestaurant(restaurantData.id);
      const foodToEdit = foodResponse.data?.find(food => food.id === parseInt(id));
      ...
      setOriginalFood(foodToEdit);
      
      // Populate form with existing data
      setFormData({
        name: foodToEdit.name || '',
        description: foodToEdit.description || '',
        price: (foodToEdit.price / 100).toString(), // Convert from cents to dollars
        category: foodToEdit.category?.id?.toString() || '',
        vegetarian: foodToEdit.vegetarian || false,
        seasonal: foodToEdit.seasonal || false, 
        ingredients: foodToEdit.ingredients || [],
        images: foodToEdit.images || []
      });
      
      // Set ingredients
      const foodIngredients = foodToEdit.ingredients?.length > 0 
        ? foodToEdit.ingredients.map(ing => ({ name: ing.name }))
        : [{ name: '' }];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const foodDataPayload = {
        ...
        images: originalFood.images || [] // Keep existing images for now
      };

      // For now, just update without new images
      // TODO: Implement proper image update functionality
      if (imageFiles.length > 0) {
        toast.warning('New image upload not yet implemented. Existing images will be kept.');
      }

      await foodAPI.update(id, foodDataPayload);
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Edit Food Item</h1>
          <p className="text-gray-600">
            Update "{originalFood?.name}" in {restaurant?.name || 'your restaurant'}
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100">
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            {/* Current Images */}
            {originalFood?.images && originalFood.images.length > 0 && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Current Images
                </label>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                  {originalFood.images.map((imageUrl, index) => (
            ...
            <div>
              ...
              <div className="text-sm text-amber-600 bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4">
                <p>⚠️ Note: New image upload functionality is not yet implemented. Existing images will be preserved.</p>
              </div>
              <div 
